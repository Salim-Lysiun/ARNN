...
train_loader, test_loader = data_generator(root, batch_size, window_length)
...
class Net(nn.Module):
    def __init__(self, sequence_length, dim, embed_dim, hidden_length, n_classes):
        super().__init__()
        
        self.hidden_length = hidden_length
        self.lstm = LSTM(sequence_length, dim, embed_dim, hidden_length) #nn.LSTM(28, 28, batch_first=True)
        self.fc1 = nn.Linear(hidden_length, n_classes)
...
#%%    
model= Net(sequence_length, dim, embed_dim, hidden_length, n_classes).to(device)


model_name = "Model_{}_sequence_length_{}_lr_{}".format(
            'LSTM',sequence_length, args.lr)
