...
x_data, label = next(iter(train_loader)) # Generate a sample data with a sigle channel
...
class RT(nn.Module):
    def __init__(self, seq_len, input_dim, embed_dim, n_class, h, rnn_type, ksize, n_level, n, dropout=0.2, emb_dropout=0.2):
        ...
        self.sequential = nn.Sequential((OrderedDict([
              ('linear1', nn.Linear(seq_len,embed_dim)),
              ('flat', nn.Flatten()),
...
#%%    
model = RT(seq_len, args.input_dim, args.embed_dim, args.n_class, h=args.h, rnn_type=args.rnn_type, ksize=args.ksize, 
...
model_name = "Model_{}_dim_{}_heads_{}_lr_{}".format(
            'RTransformer',args.embed_dim, args.heads, args.lr)
