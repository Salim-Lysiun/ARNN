...
class Vanilla_Transformer(nn.Module):
    def __init__(self, sequence_length, embed_dim, d_dim, heads, num_class, num_layers, pos_enc= False):
        super(Vanilla_Transformer, self).__init__()
        
        self.Trans = TransformerEncoder(sequence_length,
            embed_dim,d_dim, heads, num_layers, pos_enc)
    ...
    def forward(self, x):
        ...
        return out
model =  Vanilla_Transformer(sequence_length, embed_dim, d_dim, heads=2, 
                             num_class=1,num_layers=2, pos_enc= False).to(device)


model_name = "Model_{}_dim_{}_heads_{}_lr_{}_dropout_{}".format(
            'Vanilla_Transformer',args.embed_dim, args.heads, args.lr, args.dropout)
